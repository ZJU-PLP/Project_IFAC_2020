...
def print_output(n, way_points, dist_EOF_to_Goal):
...
class MoveGroupPythonIntefaceTutorial(object):
        ...
        try:
            ...
            while not rospy.is_shutdown() and i < len(way_points):
                g.trajectory.points.append(JointTrajectoryPoint(positions=way_points[i],
                                                                velocities=[0] * 6,
                                                                time_from_start=rospy.Duration(0.05 * i + 5))) #default 0.05*i + 5
                i += 1
    ...
    def trajectory_execution(self, args, point, home_pos, way_points, traj):
        ...
        if not args.COMP:
            ...
            if args.AAPF and not args.OriON:
                # Smooth path generated by AAPF
                way_points = smooth_path(way_points)

            print_output(n, way_points, dist_EOF_to_Goal)
            ...
            if args.realUR5:
                ...
                self.move(way_points, "real")
            else:
                raw_input("' =========== Press enter to send the trajectory to Gazebo \n")
                self.move(way_points, "gazebo")

        else:
            print("Generating trajectory [AAPF: ON | Orientation Control: ON]")
            AAPF_COMP = True
            ORI_COMP = True
            ...
            self.scene.clear()
            print("Generating trajectory [AAPF: OFF | Orientation Control: ON]")
            AAPF_COMP = False
            ORI_COMP = True
            ...
            self.joint_states.position = home_pos
            self.scene.clear()
            print("Generating trajectory [AAPF: ON | Orientation Control: OFF]")
            AAPF_COMP = True
            ORI_COMP = False
